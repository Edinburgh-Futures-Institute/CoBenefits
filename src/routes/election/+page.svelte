<script lang="ts">
    ...
    import {
        getAllCBAllDatazones, getAllCBForOneP_Code,
        getAverageCBGroupedByP_Code,
        getSUMCBGroupedByP_Code, getSUMCBGroupedByP_CodeAndCB,
        getTableData, getTopSelectedP_Codes,
        getTotalCBAllDatazones, getTotalCBForOneP_Code
    } from "$lib/duckdb";
    ...
    async function loadData() {
        ...
        totalCBAllZones.forEach(datazone => {
            datazone.isPageP_Code = (datazone.P_Code == P_Code) ? true : false
        });

        totalCBAllLAD = await getTableData(getSUMCBGroupedByP_Code([]));
        allCBAllLAD = await getTableData(getAverageCBGroupedByP_Code(COBENEFS.map(d => d.id)));
        allCBAllLADSUM = await getTableData(getSUMCBGroupedByP_CodeAndCB());

        oneLADData = await getTableData(getTotalCBForOneP_Code(P_Code));
        oneLADAllCbs = await getTableData(getAllCBForOneP_Code(P_Code));

        // Derive P_Name directly from the data - no CSV needed
        const nameRow = oneLADData.find(d => d.P_Name);
        P_Name = nameRow?.P_Name ?? P_Code;
        ...
        totalValuePerCapitaMax = await getTableData(getTopSelectedP_Codes({limit: 1, sortBy: "per_capita"}));
    ...
    async function initLSOAMapAfterLoad() {
        ...
        mapLSOA.setCenter(oneLADData[0].P_Code);
    }
    ...
    onMount(() => {
        // Don't create maps yet — wait for them to be bound
    ...
    // Watch for when mapDiv becomes available
    $: if (mapDiv && !mapInitialized) {
        mapInitialized = true;
        
        if (map) map.destroy();
        if (mapLSOA) mapLSOA.destroy();

        map = new MapUK(P_Code, "Westminster", mapDiv, "val", true, "PCON24CD", false, null, 8);
        map.initMap(false, false);
...
console.log("selectedDatum", selectedDatum)
    ...
    async function onChangeComparison(event) {
        ...
        allCBAllLADSUM = await getTableData(getSUMCBGroupedByP_CodeAndCB("total", compareTo));
        totalCBAllLAD = await getTableData(getSUMCBGroupedByP_Code([], compareTo));
    ...
    function exportData() {
        let data = allCBsAllZones.filter(d => d.P_Code == P_Code);
       ...
       //     delete d["val"]
        ...
        downloadCSV(csv, `cobenefits_${P_Code}.csv`);
...
<div class="page-container" bind:this={element}>
    ...
    <div class="section header header-row" id="head">
        <div>
            <div class="header-bar">
                <p class="page-subtitle">Westminster Constituency Report</p>
            </div>

            <div id="title-row">
                <h1 class="page-title"> {P_Name ?? 'Loading…'}</h1>

            </div>

            <p class="description">Explore how this Westminster constituency will benefit from achieving Net Zero and learn about
        ...
        <div class="radio-set">
            Compare this constituency against values for:
            <div class="radio-row">
    ...
    <div class="section" id="overview">
        ...
        <div id="vis-block">
            ...
            <div class="component column">
                ...
                <p class="description">Co-benefit values for {P_Name} compared to average value of benefits
                ...
                <h3 class="component-title">11 types of co-benefit values (vs. <span
                        class="nation-label">{compareTo}</span> Average)</h3>
                <p class="description">Co-benefit values for {P_Name} compared to average value of benefits
                    received across all constituencies in <span class="nation-label">{compareTo}</span> (grey).</p>
            ...
            <div class="component column">
                <h3 class="component-title">Where is {P_Name}?</h3>
                <p class="description">{P_Name} has been highlighted in dark grey on this UK map.</p>
                <div class="map-shell">
    ...
    <div class="section" id="temporal">
        ...
        <div id="vis-block">
            <div id="main-block" class="component column">
                <div>
                    ...
                    <p class="description" style="margin-bottom:5px">Aggregated values from 2025-2049
                        in {P_Name} compared to average value of benefits received across all constituencies
            ...
            <div id="main-block" class="component column">
                <div>
                    <h3 class="component-title">Co-benefit gain/loss for {P_Name} over 5 year intervals</h3>
...
<!-- Removed because LSOAS and parlimentary constituencies don't match -->
<!-- <div class="section" id="breakdown">
            ...
            <div id="main-block">
                ...
                <p class="description">The distribution plot below shows the predicted spread of benefits recieved or costs incurred by the LSOAs across {P_Name}.
            ...
            <div style="flex: 1; display: flex; flex-direction: column; padding-top: 3rem;">
                ...
                <p class="description">This map shows the total co-benefit values (£ millions) for each LSOA of {P_Name}</p>
    ...
    <div class="section" id="households">
        <div class="section-header">
            ...
            <h2 class="section-title">{P_Name} social-economic factors</h2>
        ...
        <div id="se-block" class="component" style="margin-left: 1rem;">
            <div id="se-title">
                <h3 class="component-title">Comparing the Socio-Economic factors distributions of {P_Name}
                ...
                <div id="se-legend" class="legend-box">
                    ...
                    <p><strong>Scatterplot:</strong> Each dot represents a datazone inside {P_Name}. The cloud
                                ...
                                <div class="component column">
                                    <div>
                                        ...
                                        <p class="description short">Density plot refers to UK distribution while the
                                            scattered points refer to data zones in {P_Name}.</p>
